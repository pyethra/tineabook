...
app.MapGet("/consultaUsuarios", () =>
{
    ...
    NpgsqlCommand sql = new NpgsqlCommand("SELECT * FROM usuarios",conexao);
...
app.MapPost("/alterar/{id}", ([FromBody] JsonObject dados, string id) =>
{
    ...
    NpgsqlCommand sql = new NpgsqlCommand("UPDATE aluno SET Nome = @c, email = @v, RG = @p " +
        "WHERE id = @id", conexao);
...
app.MapDelete("/excluir/{id}", ([FromBody] JsonObject dados, string id) =>
{
    ...
    NpgsqlCommand sql = new NpgsqlCommand("DELETE FROM aluno WHERE id = @id", conexao);
    sql.Parameters.AddWithValue("@id", id);
